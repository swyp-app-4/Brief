...
class ArchiveRepository(
    ...
    suspend fun getSimilarNews(newsId: Long): Result<List<NewsListItem>> {
        return try {
            val response = newsApi.getSimilarNews(newsId)
            if (response.isSuccessful) {
                Result.success(response.body() ?: emptyList())
            } else {
                Result.failure(Exception("유사 뉴스 조회 실패: ${response.code()}"))
            }
        } catch (e: Exception) {
            Result.failure(e)
        }
