...
class BrifeWidgetReceiver : AppWidgetProvider() {
    ...
    companion object {
        const val ACTION_PREV = "com.swyp.brife.WIDGET_PREV"
        const val ACTION_NEXT = "com.swyp.brife.WIDGET_NEXT"
        const val ACTION_REFRESH_HEADER = "com.swyp.brife.WIDGET_REFRESH_HEADER"
    ...
    override fun onReceive(context: Context, intent: Intent) {
        ...
        when (intent.action) {
            ...
            ACTION_NEXT -> {
                val current = getPosition(context, appWidgetId)
                val maxAccessiblePos = if (AuthLocalStorage(context).isLoggedIn()) ITEM_COUNT - 1 else 2
                if (current < maxAccessiblePos) {
                    savePosition(context, appWidgetId, current + 1)
                    updateWidget(context, appWidgetManager, appWidgetId)
                }
            }
    ...
    private fun updateWidget(
        ...
        val savedPos = getPosition(context, appWidgetId)
        val currentPos = savedPos.coerceIn(0, maxAccessiblePos)
        if (savedPos != currentPos) {
            savePosition(context, appWidgetId, currentPos)
        }
        ...
        if (isLoggedIn) {
            rv.setViewVisibility(R.id.iv_widget_bookmark, View.VISIBLE)
            rv.setImageViewResource(
            ...
            rv.setViewVisibility(R.id.iv_widget_bookmark, View.GONE)
        }
